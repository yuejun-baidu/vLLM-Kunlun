...
def _parse_gemma4_value(value_str: str) -> Any:
...
class Gemma4ToolParser(ToolParser):
    ...
    def adjust_request(self, request: ChatCompletionRequest) -> ChatCompletionRequest:
        ...
        if request.tools and request.tool_choice != "none":
            ...
            # for the parser to detect tool calls.
    ...
    def _buffer_delta_text(self, delta_text: str) -> str:
        ...
        ``<|tool_call>`` or ```` and only flushes them
    ...
    def extract_tool_calls_streaming(
        ...
        # Keep current_text from the upstream stream state. The buffered
        # delta is only for emission, and must not be stitched back into
        # the accumulated model text.
    ...
    def _extract_streaming(
        ...
        Format: ``<|tool_call>call:name{args}``
        """
    ...
    def _extract_partial_call(self, current_text: str) -> tuple[Optional[str], str]:
    ...
    def _handle_tool_call_middle(self, current_text: str) -> Union[DeltaMessage, None]:
    ...
    def _handle_tool_call_end(self, current_text: str) -> Union[DeltaMessage, None]:
    ...
    def _emit_argument_diff(self, raw_args_str: str) -> Union[DeltaMessage, None]:
        ...
        **Why withholding is necessary:**
        ...
        # Use _find_common_prefix to handle cases where the value changed
