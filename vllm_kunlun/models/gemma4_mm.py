...
from transformers.models.gemma4 import Gemma4Config, Gemma4Processor, Gemma4VisionConfig
...
from vllm_kunlun.models.gemma4 import Gemma4ForCausalLM
...
class Gemma4MultiModalProcessor(BaseMultiModalProcessor[Gemma4ProcessingInfo]):
    def _call_hf_processor(
        ...
        merged_kwargs = mm_config.merge_mm_processor_kwargs(mm_kwargs)
        ...
        if videos := mm_data.pop("videos", []):
            ...
            for item in videos:
                if isinstance(item, tuple) and len(item) == 2:
                    ...
                    # Container vLLM strips metadata before calling
                    # _call_hf_processor; synthesise a default metadata dict.
                    video_array = item
                    num_f = video_array.shape[0]
                    metadata = {
                        "fps": 2.0,
                        "duration": num_f / 2.0,
                        "total_num_frames": num_f,
                        "frames_indices": list(range(num_f)),
                        "video_backend": "opencv",
                        "do_sample_frames": False,
                    }
